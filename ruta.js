...
function route_setup(){
    ...
    for (i = 0; i < datos.features.length; i++) {
        console.log("número",datos.features.length);
        for (j = 0; j < out.shortestPaths[end_point].length; j++) {
            if (String(i) == out.shortestPaths[end_point][j]) {
                var x_i = i;
                x_path.push({
                    lat: datos.features[x_i].geometry.coordinates[1],
                    lng: datos.features[x_i].geometry.coordinates[0]
                });
            }
        }
    }
    x_path.push({
        lat: datos.features[end_point].geometry.coordinates[1],
        lng: datos.features[end_point].geometry.coordinates[0]
    });
...
function weight(a, b) {
    var R = 6371; // Radio del planeta en km
    var dLat = deg2rad(b[1] - a[1]); // Función deg2rad
...
function deg2rad(deg) {
    return deg * (Math.PI / 180); // Grado sexagesimal a radianes
}
